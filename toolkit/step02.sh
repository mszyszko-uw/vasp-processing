...
START_DIR="$(pwd)"
output=$(python3 toolkit.py --step t_geo --part dry)
cd "$output"
mpiexec vasp_std > log

cd "$START_DIR"
output=$(python3 toolkit.py --step t_geo --part cg_opt)
cd "$output"
mpiexec vasp_std > log

cd "$START_DIR"
output=$(python3 toolkit.py --step t_geo --part nw_opt)
cd "$output"
mpiexec vasp_std > log

cd "$START_DIR"
output=$(python3 toolkit.py --step t_geo --part scf)
cd "$output"
mpiexec vasp_std > log

cd "$START_DIR"
python3 toolkit.py --step t_geo --part report
