...
class vaspout_h5:
    ...
    def __init__(self, vaspout: str = 'vaspout.h5'):
        try:
            ...
            vaspout_path = vaspout + 'vaspout.h5'
            try:
                self.vaspout = h5py.File(vaspout_path, 'r')
            except FileNotFoundError:
                raise FileNotFoundError(f'No vaspout.h5 file found at: {vaspout_path}')
        except FileNotFoundError:
            raise FileNotFoundError(f'No vaspout.h5 file found at: {vaspout}')
    ...
    def read_EIGENVAL(self):
        with open(self.EIGENVAL) as eig:
            ...
            for i in range(5):
                eig.readline()
            data = np.array(eig.readline().split(), dtype=int)
    ...
    def Optical_Selection_Rules(self, kpoint, vb_max, num_vb, num_cb, save_txt = True, save_np = False, **kwargs):
        ...
        nkpts, nb_tot, nbands_cder = (self.ic.WDR_matrix.shape[0],
                                       self.ic.WDR_matrix.shape[1],
                                       self.ic.WDR_matrix.shape[2])
        if kpoint < 1 or kpoint > nkpts:
            raise IndexError(f'kpoint={kpoint} out of range [1, {nkpts}]')
        if vb_max < 1 or vb_max > nb_tot:
            raise IndexError(f'vb_max={vb_max} out of range [1, {nb_tot}]')
        if vb_max - num_vb + 1 < 1:
            raise IndexError(f'num_vb={num_vb} too large for vb_max={vb_max}')
        if vb_max + num_cb > nbands_cder:
            raise IndexError(f'num_cb={num_cb} exceeds available conduction bands above vb_max={vb_max}')
    ...
    # direction: 1 - X, 2 - Y, 3 - Z
        ...
        nkpts_wdr = self.ic.WDR_matrix.shape[0]
        nbands = self.ic.WDR_matrix.shape[1]
        nbands_cder = self.ic.WDR_matrix.shape[2]
        if kpoint < 1 or kpoint > nkpts_wdr:
            raise IndexError(f'kpoint={kpoint} out of range [1, {nkpts_wdr}]')
        if vb_max - num_vb + 1 < 1:
            raise IndexError(f'num_vb={num_vb} too large for vb_max={vb_max}')
        if vb_max + num_cb > nbands_cder:
            raise IndexError(f'num_cb={num_cb} exceeds available conduction bands above vb_max={vb_max}')
